...
:: ====================================================
:: Portable llama.cpp Unsloth Fork Installer
:: ====================================================
...
echo ====================================================
echo       INSTALLING PORTABLE LLAMA.CPP UNSLOTH
echo ====================================================
...
set "_PS_FILE=%TEMP%\portable_llamacpp_unsloth_install.ps1"
...
if errorlevel 1 (
echo ====================================================
echo       LLAMA.CPP UNSLOTH INSTALL FAILED
echo ====================================================
...
echo ====================================================
echo       LLAMA.CPP UNSLOTH INSTALL COMPLETE
echo ====================================================
del "%_PS_FILE%" >nul 2>&1
...
echo $UnslothDownloadRoot = Join-Path $DownloadRoot "llama.cpp unsloth fork"
echo.
echo if (-not (Test-Path $DownloadRoot)) {
echo     New-Item -ItemType Directory -Path $DownloadRoot -Force ^| Out-Null
echo }
echo.
echo if (-not (Test-Path $UnslothDownloadRoot)) {
echo     New-Item -ItemType Directory -Path $UnslothDownloadRoot -Force ^| Out-Null
echo }
echo.
echo Write-Host "Finding latest Unsloth llama.cpp release"
echo.
echo $ReleaseApi = "https://api.github.com/repos/unslothai/llama.cpp/releases/latest"
echo.
echo $Headers = @{
...
echo.
echo if (-not $Asset) {
echo     throw "No matching Unsloth build found for backend: $Backend"
echo }
echo.
echo $Archive = Join-Path $UnslothDownloadRoot $Asset.name
echo.
...
echo     Write-Host "Downloading Unsloth llama.cpp"
...
echo.
echo     $ExtractDir = Join-Path $Root ".llamacpp-unsloth-extract"
echo.
...
echo.
echo     Write-Host "Extracting Unsloth llama.cpp"
