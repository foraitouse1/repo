...
REM ============================================================
REM FLUX.2 KLEIN 4B - IMAGE EDIT
REM ============================================================

set "INPUT=%SCRIPT_DIR%\test.jpg"
set "OUTPUT=%SCRIPT_DIR%\test-edit.jpg"
set "PROMPT=Change the sky to yellow. Keep background, lighting, and composition unchanged. make sure not to crop the image."
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$image=[Convert]::ToBase64String([IO.File]::ReadAllBytes('%INPUT%')); $body=@{prompt='%PROMPT%'; width=[int]%WIDTH%; height=[int]%HEIGHT%; ref_images=@($image); sample_params=@{sample_steps=[int]%STEPS%}} | ConvertTo-Json -Depth 10; Write-Host ''; Write-Host 'Submitting FLUX.2 Klein edit...'; Write-Host ('Input : %INPUT%'); Write-Host ('Output: %OUTPUT%'); Write-Host ('Size  : %WIDTH%x%HEIGHT%'); Write-Host ('Steps : %STEPS%'); Write-Host ('Prompt: %PROMPT%'); Write-Host ''; $job=Invoke-RestMethod -Uri '%API%' -Method Post -ContentType 'application/json' -Body $body; Write-Host ('Job ID: '+$job.id); do { Start-Sleep -Milliseconds 500; $result=Invoke-RestMethod -Uri ('%JOB_API%/'+$job.id); Write-Host ('Status: '+$result.status) } while ($result.status -ne 'completed'); [IO.File]::WriteAllBytes('%OUTPUT%',[Convert]::FromBase64String($result.result.images[0].b64_json)); Write-Host ''; Write-Host '========================================'; Write-Host 'DONE'; Write-Host ('Output: '+$env:OUTPUT); Write-Host '========================================'"
