...
>> "%PS_SCRIPT%" echo if ($debug) { Write-Host "[DEBUG-PS] Value: '$value'" -ForegroundColor Green }
>> "%PS_SCRIPT%" echo.
...
>> "%PS_SCRIPT%" echo                 if ($lines.Count -ge 2) {
>> "%PS_SCRIPT%" echo                     $existingSet = $lines[1].Trim()
>> "%PS_SCRIPT%" echo                     if ($existingSet -match '^(?i)set\s+"?([^"=]+)"?\s*=') {
>> "%PS_SCRIPT%" echo                         $existingVar = $Matches[1].Trim()
>> "%PS_SCRIPT%" echo                         $blocks += [PSCustomObject]@{ Name = $existingVar; Content = ($lines -join "`r`n") }
>> "%PS_SCRIPT%" echo                     }
>> "%PS_SCRIPT%" echo                 }
>> "%PS_SCRIPT%" echo             }
>> "%PS_SCRIPT%" echo         }
...
>> "%PS_SCRIPT%" echo }
>> "%PS_SCRIPT%" echo.
>> "%PS_SCRIPT%" echo $existing = $blocks ^| Where-Object { $_.Name -eq $varName }
>> "%PS_SCRIPT%" echo.
...
>> "%PS_SCRIPT%" echo.
>> "%PS_SCRIPT%" echo     if ($existing.Content.Trim() -eq $newBlock.Trim()) {
>> "%PS_SCRIPT%" echo         if ($debug) { Write-Host '[DEBUG-PS] Existing block is identical. No changes needed.' -ForegroundColor Green }
...
>> "%PS_SCRIPT%" echo }
>> "%PS_SCRIPT%" echo.
>> "%PS_SCRIPT%" echo $blocks += [PSCustomObject]@{ Name = $varName; Content = $newBlock }
